...
from vao import prepare_vao_cube, prepare_vao_cube_, prepare_vao_frame, prepare_vao_pivot, prepare_vao_grid
import input as inp
...
def draw_cube_array(vao, MVP, loc_MVP):
    ...
    for i in range(5):
        for j in range(5):
            for k in range(5):
                MVP_cube = MVP * glm.translate(glm.vec3(i, j, k)) * glm.scale(glm.vec3(.5,.5,.5))
...
def main():
    ...
    while not glfwWindowShouldClose(window):
        ...
        # rotation
        spin = np.radians(inp.obj_spin*90)
        R = glm.rotate(spin, glm.vec3(0,0,1))

        # translation
        T = glm.translate(glm.vec3(inp.obj_trans_x, inp.obj_trans_y, 0.))
        ...
        # # draw cube w.r.t. the current frame
        ...
        # draw pivot point at camera rotation center
        MVP = P*V*glm.translate(glm.vec3(inp.g_pan_x, inp.g_pan_y, inp.g_pan_z))
        glUniformMatrix4fv(loc_MVP, 1, GL_FALSE, glm.value_ptr(MVP))
        glPointSize(10)
